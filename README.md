...
To install Aftermath Blame Assigner on these distros, run the installer script using `sudo bash aba_install.sh`. On any other distro, please follow the manual install instructions below. Installation on any other UNIX-like OS, such as \*BSD or macOS will have to be done manually as well, but because of the limited functionality of the script on these OSs installation is not recommended.
